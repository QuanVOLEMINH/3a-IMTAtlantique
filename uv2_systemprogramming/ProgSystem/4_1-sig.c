...
void handler_msg1 (int n){
	printf("message 1 \n");
}

void handler_msg2 (int n){
	printf("message 2 \n");
}

void handler_signal_quit(int n){
...
int main() {
  ...
  // void (*sig_avant)(int) = &sig_handler;
  signal(SIGQUIT, handler_signal_quit);
  signal(SIGINT, handler_msg1);
  sig_avant = handler_msg2;
