...
int main() {
  ...
  struct sigaction nouvelle_action, ancienne_action;
 
  sigemptyset(&nouvelle_action.sa_mask);
  nouvelle_action.sa_flags = 0;

  nouvelle_action.sa_handler = hdl_sys1;
 
  sigaction(SIGQUIT, &nouvelle_action, &ancienne_action);
