...
int main(int argc, char **argv) {
	...
	for (;;) {
		...
		if (FD_ISSET(server, &rd_set)) {	/* on a une nouvelle connection */
			...
			r = accept(server, (struct sockaddr *)&client, &clientlen);
			if (r < 0) {
				perror("accept()");
			} else {
				printf("accepté %d \n", r);
				...
				if (i < max_clients) {
					...
					printf("Plus de clilen = sizeof(cli_addr);place pour un nouveau client.\n");
					close(r);
				}
			}
		}
		...
		for (i=0; i<max_clients; i++) {
			if ( (clients[i] > 0) && FD_ISSET(clients[i], &rd_set)) {
				...
				if (rd_sz < 0) {
					...
					printf("Reception de %ld octets du client %d : [\n", rd_sz, i);
					/* On ecrit sur la sortie standard */
					write(server, buffer, BUF_SIZE);
					printf("]\n");
					/* Envoie le paquet (en un seul coup) a tous les autres clients */
					/*for (j=0; j<max_clients; j++) {
						if ( (clients[j] > 0) && (i != j) ) {
							wr_sz = send(server, "ok", sizeof("ok"), 0);
							if ( wr_sz < 0 ) {
								...
								printf("Envoie de %ld octets au client %d.\n", wr_sz, j);
						}
					}*/
				}
			}
