...
public class Stock {
	...
	static String[][] leStock = {
			...
			{ "Chansons Ordinaires", "Christophe Miossec", "25", "disque006542008", "2008",
					"ChristopheOrdinaires.jpg" },
	...
	public static Disque trouveDisque(String reference) {
		for (int i = 0; i < leStock.length; i++) {
			if (leStock[i][3].equals(reference))
				return new Disque(leStock[i][0], leStock[i][1], Integer.parseInt(leStock[i][2]), leStock[i][3],
						Integer.parseInt(leStock[i][4]), leStock[i][5]);
		}
		return null;
	}

	public static Disque donneDisque(int numero) {
		return new Disque(leStock[numero][0], leStock[numero][1], Integer.parseInt(leStock[numero][2]),
				leStock[numero][3], Integer.parseInt(leStock[numero][4]), leStock[numero][5]);
	}
